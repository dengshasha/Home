...
// import ModifySchemePage from './modifySchemePage';
var deviceWidth = common.getWidth();
var deviceHeight = common.getHeight();

export default class CommunitySchemeDetailPage extends Component {
  constructor(props){
    super(props);
    this.state = {
  ...
  renderButtons(){
    let menuData = [
      {
        image: require('../../images/panorama/icon_save_dna.png'),
        onPress: ()=>this.props.navigator.push({id: 'SaveDna', component: SaveDna}),
        text: '收藏风格'
      },
      {
        image: require('../../images/panorama/icon_collect_dna.png'),
        onPress: ()=>this.props.navigator.push({id: 'SaveScheme', component: SaveScheme}),
        text: '收藏方案'
      },
      {
        image: require('../../images/panorama/icon_thumbs_up.png'),
        onPress: ()=>this.likeScheme(),
        text: '点赞'
      },
      {
        image: require('../../images/panorama/activity.png'),
  ...
  renderWebview() {
    let url = SchemeHandler.getPanoramaUrl() ;
    if(Platform.OS === 'ios'){
      return(
        <WebView
          ref="webview"
