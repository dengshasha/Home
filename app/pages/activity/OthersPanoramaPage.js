...
export default class OthersPanoramaPage extends Component {
  constructor(props){
    super(props);
    this.state = {
  ...
  componentDidMount(){
    if (SchemeHandler.scheme && SchemeHandler.scheme.id) {
  ...
  renderWebview() {
    let url = this.props.route.PanoramaUrl ? this.props.route.PanoramaUrl : SchemeHandler.getPanoramaUrl();
    if(Platform.OS === 'ios'){
      return(
        <WebView
          ref="webview"
      ...
      <View style = {{flex: 1}}>
        ...
        <View style = {{position: 'absolute', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',top: this.isPortrait()?20:0, left: 0, right: 0, height: 44, width: deviceWidth}}>
          <TouchableOpacity onPress = {()=> this.props.navigator.pop()}>
            <Image style = {{margin: 15}} source = {require('../../images/common/icon_back_white.png')}/>
          </TouchableOpacity>
        </View>
        {/**菜单栏**/}
